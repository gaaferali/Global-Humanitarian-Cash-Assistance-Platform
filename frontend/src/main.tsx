...
const roleRoutes: Record<Route, Role[]> = {
  ...
  households: ["ADMIN", "FIELD_OFFICER"],
  beneficiaries: ["ADMIN", "FIELD_OFFICER", "REVIEWER"],
  eligibility: ["ADMIN", "REVIEWER"],
  enrollment: ["ADMIN", "REVIEWER"],
...
function Textarea({ label, name, required = false, help }: { label: string; name: string; required?: boolean; help?: string }) {
  const translate = useTranslation();
  const id = `field-${name}`;
  return <div className="mb-3"><label className="form-label" htmlFor={id}>{translate(label)}{required && " *"}</label><textarea className="form-control" id={id} name={name} required={required} rows={3} />{help && <div className="form-text">{translate(help)}</div>}</div>;
}
...
function Intake({ beneficiary, role }: { beneficiary: boolean; role: Role }) {
  ...
  const [records, setRecords] = useState<Entity[]>([]); const [programs, setPrograms] = useState<Entity[]>([]); const [households, setHouseholds] = useState<Entity[]>([]); const [selectedHousehold, setSelectedHousehold] = useState(() => beneficiary ? sessionStorage.getItem("hcap_selected_household_id") ?? "" : ""); const [createdHousehold, setCreatedHousehold] = useState<Entity | null>(null); const [status, setStatus] = useState(""); const [error, setError] = useState("");
...
function PdmPage({ user }: { user: PlatformUser }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const formElement = event.currentTarget; const form = new FormData(formElement); setStatus("saving"); setError(""); try { await api.create("pdm", { program: form.get("program"), received_rate: form.get("received_rate"), amount_received: form.get("amount_received") || 0, access_problem_rate: form.get("access_problem_rate") || 0, complaint_rate: form.get("complaint_rate") || 0, satisfaction: form.get("satisfaction") || 0 }); formElement.reset(); await load(); setStatus("saved"); } catch (reason) { setStatus("failed"); setError(reason instanceof Error ? reason.message : "Could not submit the PDM response."); } };
  const loadSummary = async () => { const query = new URLSearchParams(); if (user.is_superuser && tenantId) query.set("tenant", tenantId); if (programId) query.set("program", programId); try { setError(""); setSummary(await api.get(`/pdm/summary/${query.toString() ? `?${query}` : ""}`)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not load PDM summary."); } };
  ...
  return <div className="row g-3">{!readOnly && <div className="col-lg-5"><Panel title="Post-Distribution Monitoring"><p className="form-text">Record and review whether assistance reached people as intended after distribution.</p><form onSubmit={submit}><SelectField label="Program" name="program" options={visiblePrograms.map((program) => ({ value: program.id, label: programLabel(program) }))} value={programId} onChange={setProgramId} required /><Field label="Received rate" name="received_rate" type="number" required /><Field label="Amount received" name="amount_received" type="number" /><Field label="Access problem rate" name="access_problem_rate" type="number" /><Field label="Complaint rate" name="complaint_rate" type="number" /><Field label="Satisfaction" name="satisfaction" type="number" /><button className="btn btn-primary">Submit PDM response</button></form><Message status={status} error={error} /></Panel></div>}<div className={readOnly ? "col-12" : "col-lg-7"}><Panel title="PDM summary"><div className="row"><div className="col-md-6">{user.is_superuser && <SelectField label="Tenant" name="pdm-tenant" options={tenants.map((tenant) => ({ value: tenant.id, label: tenant.name ?? "Unnamed tenant" }))} value={tenantId} onChange={(value) => { setTenantId(value); setProgramId(""); }} />}</div><div className="col-md-6"><SelectField label="Program" name="pdm-program" options={visiblePrograms.map((program) => ({ value: program.id, label: programLabel(program) }))} value={programId} onChange={setProgramId} /></div></div><button className="btn btn-outline-primary" onClick={() => void loadSummary()}>View calculated summary</button>{summary && <Table headings={["Selected program", "Responses", "Channel", "Operational location", "Received rate", "Amount received", "Access problems", "Complaint rate", "Satisfaction"]} rows={[[String((summary.program as Record<string, unknown> | null)?.name ?? "All programs"), String(summary.responses ?? 0), summaryChannels, summaryLocations, String(summary.received_rate ?? 0), String(summary.amount_received ?? 0), String(summary.access_problem_rate ?? 0), String(summary.complaint_rate ?? 0), String(summary.satisfaction ?? 0)]]} />}</Panel><Panel title="PDM responses"><Table headings={["Program", "Channel", "Operational location", "Received rate", "Amount received", "Satisfaction"]} rows={responses.map((response) => [valueOf(response, "program__name") || "Program", valueOf(response, "channel") || "Not available", valueOf(response, "location") || "Not available", valueOf(response, "received_rate"), valueOf(response, "amount_received"), valueOf(response, "satisfaction")])} /></Panel><Message status="" error={error} /></div></div>;
}
...
function AutomationPage({ role }: { role: Role }) {
  ...
  return <div className="row g-3">
    ...
    {canReconcile && <><div className="col-lg-6"><Panel title="Advisory risk review"><form onSubmit={(event) => run(event, "/ai/anomalies/", (form) => ({ batch_id: String(form.get("batch")) }))}><SelectField label="Simulated payment batch" name="batch" options={batches.map((batch) => ({ value: batch.id, label: `${batch.name ?? "Batch"} — ${valueOf(batch, "program_name")}` }))} required /><button className="btn btn-outline-primary">Run advisory risk scan</button></form></Panel></div><div className="col-lg-6"><Panel title="Simulated reconciliation"><form onSubmit={(event) => run(event, "/ai/reconcile/", (form) => { try { return { batch_id: String(form.get("batch")), provider_report_data: JSON.parse(String(form.get("provider_report_data") || "{}")) }; } catch { throw new Error("Provider report must be valid JSON."); } })}><SelectField label="Simulated payment batch" name="batch" options={batches.map((batch) => ({ value: batch.id, label: `${batch.name ?? "Batch"} — ${valueOf(batch, "program_name")}` }))} required /><Textarea label="Controlled simulated provider report" name="provider_report_data" required help='Example: {"SIM-reference":{"amount":"100","status":"SUCCESS"}}' /><button className="btn btn-outline-primary">Reconcile simulated batch</button></form></Panel></div></>}
